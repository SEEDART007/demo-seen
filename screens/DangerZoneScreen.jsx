...
const DEEPGRAM_API_KEY = '439e9d68b1482b32378dd2f10c957276d906430f'; // Store securely
...
const HelpDetectionScreen = () => {
  ...
  const [triggerWord, setTriggerWord] = useState('hello');

  const pulseAnim = useRef(new Animated.Value(1)).current;
  const stopFlag = useRef(false);
  const triggerWordRef = useRef('hello');
  ...
  const startRecording = async () => {
    if (stopFlag.current || !inDangerZoneRef.current || recordingInProgressRef.current) return;
    try {
  ...
  const startRecordingLoop = async () => {
    const filePath = `${RNFS.DocumentDirectoryPath}/recording.mp3`;
    recordingInProgressRef.current = true;

    while (!stopFlag.current && inDangerZoneRef.current) {
      try {
        if (await RNFS.exists(filePath)) await RNFS.unlink(filePath);

        if (!inDangerZoneRef.current) break;
        ...
        if (transcript.toLowerCase().includes(triggerWordRef.current.toLowerCase())) {
          Alert.alert('🆘 Trigger Detected!', `Detected the word "${triggerWordRef.current}" in speech.`);
  ...
  const sendEmergencySMS = async () => {
    ...
    Geolocation.getCurrentPosition(
      async (position) => {
        ...
        const message = `🆘 Emergency! Trigger word "${triggerWordRef.current}" detected.\nLocation: ${mapsUrl}`;

        for (const contact of contacts) {
          try {
            await axios.post(
              'https://riseup-sms-6262.twil.io/welcome',
              { to: contact, message }
            );
          } catch (error) {
        ...
        Alert.alert('📩 SMS Sent', 'Emergency messages have been sent to your contacts.');
      },
      (error) => {
    ...
    <View style={styles.container}>
      ...
      <View style={styles.content}>
        <Animated.View style={[
          ...
          currentZone && styles.dangerStatus
        ]}>
          <Text style={styles.statusIcon}>
            {currentZone ? '⚠️' : '✅'}
          </Text>
          <Text style={styles.statusText}>
            {currentZone ? 'In Danger Zone' : 'In Safe Area'}
          </Text>
        </Animated.View>
        ...
        <View style={styles.recordingBox}>
          <Text style={styles.recordingText}>
            {isRecording ?
              `Audio recording active in ${currentZone?.name}` :
        ...
        {isRecording && (
          <TouchableOpacity style={styles.stopButton} onPress={handleManualStop}>
      ...
      <View style={styles.footer}>
        <Text style={styles.footerText}>
          {currentZone ?
            ...
            "Will auto-activate in danger zones"}
        </Text>
      </View>
...
const styles = StyleSheet.create({
  ...
  subtitle: { color: '#e94560', fontSize: 14, marginTop: 5 },
  ...
  dangerStatus: { backgroundColor: 'rgba(234, 67, 53, 0.1)', borderColor: '#ea4335' },
  statusIcon: { fontSize: 50, marginBottom: 10 },
  statusText: { color: '#fff', fontSize: 18, fontWeight: '600' },
  locationBox: { backgroundColor: 'rgba(255, 255, 255, 0.1)', padding: 15, borderRadius: 10, marginBottom: 20 },
  locationText: { color: '#fff', fontSize: 14 },
  recordingBox: { backgroundColor: 'rgba(0, 0, 0, 0.3)', padding: 15, borderRadius: 10, marginBottom: 20 },
  recordingText: { color: '#fff', fontSize: 16, marginBottom: 5, textAlign: 'center' },
  triggerWordText: { color: '#e94560', fontSize: 14, textAlign: 'center' },
  stopButton: { backgroundColor: '#ea4335', paddingVertical: 15, paddingHorizontal: 30, borderRadius: 25, marginTop: 10 },
  stopButtonText: { color: '#fff', fontSize: 16, fontWeight: 'bold' },
  footer: { padding: 20, borderTopWidth: 1, borderTopColor: 'rgba(255, 255, 255, 0.1)', alignItems: 'center' },
  footerText: { color: '#e94560', fontSize: 14 },
  permissionWarning: { backgroundColor: '#ff6b6b', padding: 15, alignItems: 'center' },
  warningText: { color: '#fff', fontSize: 14, textAlign: 'center' },
});
